import React from 'react';

const Projects = () => {
  return (
    <div id="projects"  className="md:ml-16 project">

      <p className="mt-20 md:ml-0 ml-10 text-2xl text-white font-bold">Projects</p>

      <card className="flex flex-wrap card-project">

        <div className="flex-col justify-center items-center p-6 m-5 bg-slate-900 w-[300px] h-[100%] object-cover">
          <img
            src="https://i.pinimg.com/564x/37/25/ce/3725ce5d9c47b16c79fdafd4808171a2.jpg"
            className="w-[300px] h-[100px] object-cover" />
          <div className=" text-white mt-2 ">
            <p className='text-yellow-100 font-bold'>ViewVibes</p>
            <p>This is a streaming platform where you can access unlimited movies and live shows</p>
          </div>
        </div>


        <div className="flex-col justify-center items-center p-6 m-5 bg-slate-900 w-[300px] h-[100%] object-cover">
          <img
            src="https://i.pinimg.com/564x/37/25/ce/3725ce5d9c47b16c79fdafd4808171a2.jpg"
            className="w-[300px] h-[100px] object-cover" />
          <div className=" text-white mt-2 ">
            <p className='text-yellow-100 font-bold'>ViewVibes</p>
            <p>This is a streaming platform where you can access unlimited movies and live shows</p>
          </div>
        </div>



        <div className="flex-col justify-center items-center p-6 m-5 bg-slate-900 w-[300px] h-[100%] object-cover">
          <img
            src="https://i.pinimg.com/564x/37/25/ce/3725ce5d9c47b16c79fdafd4808171a2.jpg"
            className="w-[300px] h-[100px] object-cover" />
          <div className=" text-white mt-2 ">
            <p className='text-yellow-100 font-bold'>ViewVibes</p>
            <p>This is a streaming platform where you can access unlimited movies and live shows</p>
          </div>
        </div>


        <div className="flex-col justify-center items-center p-6 m-5 bg-slate-900 w-[300px] h-[100%] object-cover">
          <img
            src="https://i.pinimg.com/564x/37/25/ce/3725ce5d9c47b16c79fdafd4808171a2.jpg"
            className="w-[300px] h-[100px] object-cover" />
          <div className=" text-white mt-2 ">
            <p className='text-yellow-100 font-bold'>ViewVibes</p>
            <p>This is a streaming platform where you can access unlimited movies and live shows</p>
          </div>
        </div>



        <div className="flex-col justify-center items-center p-6 m-5 bg-slate-900 w-[300px] h-[100%] object-cover">
          <img
            src="https://i.pinimg.com/564x/37/25/ce/3725ce5d9c47b16c79fdafd4808171a2.jpg"
            className="w-[300px] h-[100px] object-cover" />
          <div className=" text-white mt-2 ">
            <p className='text-yellow-100 font-bold'>ViewVibes</p>
            <p>This is a streaming platform where you can access unlimited movies and live shows</p>
          </div>
        </div>

        <div className="flex-col justify-center items-center p-6 m-5 bg-slate-900 w-[300px] h-[100%] object-cover">
          <img
            src="https://i.pinimg.com/564x/37/25/ce/3725ce5d9c47b16c79fdafd4808171a2.jpg"
            className="w-[300px] h-[100px] object-cover" />
          <div className=" text-white mt-2 ">
            <p className='text-yellow-100 font-bold'>ViewVibes</p>
            <p>This is a streaming platform where you can access unlimited movies and live shows</p>
          </div>
        </div>

      </card>


    </div>
  );
};

export default Projects;


